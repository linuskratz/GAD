...
public class DynamicArray {
	...
	/**
	 * Diese Methode erlaubt es dem Benutzer, das dynamische Feld über Änderungen im
	 * verwendeten Feldbereich zu informieren. Die Methode passt ggf. den internen
	 * Speicher an (Reallocation). Elemente dürfen dabei umkopiert und der
	 * verwendete Bereich verschoben werden. Die Methode gibt zurück, wo sich die
	 * Elemente, die sich vor dem Aufruf in Verwendung befanden, nach dem Aufruf
	 * befinden.
	 * 
	 * Bezüglich der Speicherverwaltung, die z.B. mit Hilfe einer privaten
	 * Hilfsfunktion implementiert werden kann, gelten die folgenden Regeln:
	 * 
	 * - Wird aktuell mehr Speicher benötigt, als vorhanden ist, wird die Größe des
	 ...
	 * [Speicherforderung]*[Wachstumsfaktor] viele Elemente fassen kann.
	 * 
	 * @param usage   gibt an, welche Elemente des dynamischen Feldes aktuell in
	 ...
	 * 
	 * @param minSize gibt die minimale Größe benötigten Feldbereiches nach dem
	...
	public Interval reportUsage(Interval usage, int minSize) {
		...
		if (usage.getTo() != 0 || usage.getFrom() != 0) {
			int x = usage.getTo() - usage.getFrom() % elements.length;
			if (x < 0) {
				x = x + elements.length;
			}
			usageLength = x + 1;
		...
		if (minSize > elements.length || elements.length > maxOverhead * minSize) {
			newE = new int[minSize * growthFactor];
			if (newE.length != 0) {
				if (usage.getTo() < usage.getFrom()) {
					int endArr = elements.length - usage.getFrom();
					int elementsBegin = usageLength - endArr;
					System.arraycopy(elements, usage.getFrom(), newE, 0, endArr);
					System.arraycopy(elements, 0, newE, endArr, elementsBegin);
				} else {
					System.arraycopy(elements, usage.getFrom(), newE, 0, usageLength);
				}
				if (usageLength != 0)
					usage = new NonEmptyInterval(0, usageLength - 1);
			}else {
	...
	/**
	 * Diese Methode setzt des Wert des dynamischen Feldes an einem bestimmten Index
	 * auf einen Wert.
	 * 
